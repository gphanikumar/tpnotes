...
\begin{center}
\framebox{
\includegraphics[scale=0.5]{images/c16-Gemini-BoxFurnace.eps}
}
\end{center}
\caption{Box type multi-walled furnace schematic created using Gemini 2.0 Flash.}
\label{boxfurnace}
\end{figure}

In the figure~\ref{boxfurnace}, the multi-walled furnace is shown schematically. The wall comprises of a ceramic layer, glass wool layer and then a steel sheet that forms the furnace box. One can estimate the heat loss through this composite wall at steady state as follows.
...
$$ \frac{q}{A} = h_a \left(T_a - T_1 \right) = 
      -k_{12} \frac{T_2 - T_1}{\Delta x_{12}} = 
      -k_{23} \frac{T_3 - T_2}{\Delta x_{23}} = 
      -k_{34} \frac{T_4 - T_3}{\Delta x_{34}} = 
       h_b \left(T_4 - T_b \right) $$

In the figure~\ref{boxfschematic}, $k_{12}$ is thermal conductivity of the ceramic wall of width $\Delta x_{12}$, $k_{23}$ is thermal conductivity of the glass wool wall of width $\Delta x_{23}$, $k_{34}$ is thermal conductivity of the steel wall of width $\Delta x_{34}$. The heat transfer coefficients are $h_a$ for inner wall and $h_b$ for the outer wall. The ambient temperature is $T_b$ and the furnace temperature is $T_a$.
...
\begin{center}
\framebox{
\includegraphics[scale=0.5]{images/c16-BoxFurnaceSchematic.eps}
}
\end{center}
\caption{Schematic thermal profile across a composite wall of a furnace. The image of the furnace was created using Gemini 2.0 Flash.}
\label{boxfschematic}
\end{figure}
...
\frac{\ln{\frac{r_2}{r_1}}}{k_{12}} + \frac{\ln{\frac{r_3}{r_2}}}{k_{23}} +
\frac{\ln{\frac{r_4}{r_3}}}{k_{34}} + \frac{1}{r_4 h_4} \right] $$

The radial distances of the walls from the centre for the tubular furnace are given as $r_1$, $r_2$ etc., The length of the tubular furnace is $L$. Rest of the parameters are simialar to the rectangular case above.

\section{Summary}

\begin{enumerate}

\item Thermal resistances for rectangular coordinate system are: $1 \over {A h}$ for convective boundary condition, $\Delta x \over {Ak}$ for conduction across a slab of thickness $\Delta x$.

\item Thermal resistances for cylindrical coordinate system are: $1 \over {2\pi L r h}$ for convective boundary condition, $ \log{r_2 \over r_1} \over {2 \pi L r_1 k}$ for conduction across a cylindrical slab of thickness $r_2 - r_1$ considering heat flux at the inner wall at radial distance $r_1$.

\item As the radius of curvature becomes very large, a cylindrical or spherical shell can be assumed to be rectangular slab for constant heat flow across it.
